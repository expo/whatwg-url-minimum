...
(function () {
  ...
  IdlArray.prototype.internal_add_dependency_idls = function (
    ...
    const defined_or_untested = name => {
      ...
      return name in this.members || this.is_excluded_by_options(name, options);
    };
  ...
  IdlArray.prototype.test = function () {
    ...
    Object.getOwnPropertyNames(this.members).forEach(
      function (memberName) {
        ...
        if (
          !(member instanceof IdlInterface || member instanceof IdlNamespace)
        ) {
          return;
        }
  ...
  IdlInterface.prototype.has_default_to_json_regular_operation = function () {
    return this.members.some(function (m) {
      return (
        m.is_to_json_regular_operation() && m.has_extended_attribute('Default')
      );
    });
  };
      ...
      if (!this.untested) {
        subsetTestByKey(
          ...
          function () {
            assert_false(
              this.name in self,
              this.name + ' interface should not exist'
            );
          }.bind(this),
    ...
    subsetTestByKey(
      ...
      function () {
        ...
        if (unscopables.length != 0) {
          ...
          assert_false(
            desc.writable,
            this.name + '.prototype[Symbol.unscopables] should not be writable'
          );
          assert_false(
            desc.enumerable,
            this.name +
          ...
          assert_true(
            desc.configurable,
            this.name + '.prototype[Symbol.unscopables] should be configurable'
          );
          assert_equals(
            desc.value,
      ...
      if (!this.untested) {
        subsetTestByKey(
          ...
          function () {
            assert_false(
              this.name in self,
              this.name + ' namespace should not exist'
            );
          }.bind(this),
          this.name + ' namespace: existence and properties of namespace object'
        );
      }
      return;
    }
...
function idl_test(srcs, deps, idl_setup_func) {
  return promise_test(function (t) {
    ...
    return Promise.all(srcs.concat(deps).map(globalThis.fetch_spec))
